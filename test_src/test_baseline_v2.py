#!/usr/bin/env python3
"""
Test Phase 1 baseline: Train DegreeSignatureNN on original graph.

This script trains the baseline DegreeSignatureNN model on the original
Hetionet graph and validates on permutation averages.

Usage:
    python test_src/test_baseline_v2.py
    python test_src/test_baseline_v2.py --metapath CtDaG
    python test_src/test_baseline_v2.py --n-bins 15 --epochs 1000
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Train baseline DegreeSignatureNN on original graph'
    )
    parser.add_argument(
        '--metapath',
        default='CbGpPW',
    ...
    results_dir = repo_dir / 'results' / 'baseline_v2'
    ...
    # Step 1: Extract features from original graph
    print("Step 1: Extracting features from original Hetionet graph...")
    print("  Loading from: data/edges/")
    X_train, y_train, metadata = extract_features_from_original(
        edge1_type, edge2_type, data_dir, args.n_bins, feature_set='A'
    )
    ...
    # Split into train/test for monitoring during training
    ...
    # Step 3: Train model (with test loss monitoring, but no early stopping)
    ...
    print(f"  Note: Training on all data to match original Phase 1 performance")
    print()

    # Train on ALL data for final model, but track test loss for visualization
    training_results = train_model(
        ...
        early_stopping_patience=1000,  # Effectively disable early stopping
        device=device,
        verbose=True
    )
