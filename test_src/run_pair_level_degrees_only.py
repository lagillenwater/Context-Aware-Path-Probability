"""
Pair-Level Pathway Prediction with Degrees Only (No Jaccard)

Train Linear Regression on original Hetionet using only degree features.
Validate on permutation data.

Goal: Test if degree features alone can predict null distributions.
...
results_dir = repo_dir / 'results' / 'pair_level_degrees_only'
results_dir.mkdir(parents=True, exist_ok=True)

print("="*80)
print("PAIR-LEVEL PATHWAY PREDICTION WITH DEGREES ONLY")
print("="*80)
...
def extract_degree_features(source_idx, target_idx, edge1, edge2):
    """
    Extract degree features only (no Jaccard).
...
results_df.to_csv(results_dir / 'degrees_only_results.csv', index=False)
print(f"\n  Saved results to {results_dir / 'degrees_only_results.csv'}")
...
plt.tight_layout()
plt.savefig(results_dir / 'degrees_only_analysis.png', dpi=150, bbox_inches='tight')
print(f"    Saved visualization to {results_dir / 'degrees_only_analysis.png'}")
...
if r_test > 0.85 and r_val > 0.85:
    ...
    print(f"\n  Test r = {r_test:.4f}, validation r = {r_val:.4f}")
